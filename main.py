...
def findLast30Days(local_obj, name, numOfDay):
    # inner function
    def setDict(url='', address='', business_name='', category_snippet='', departments='', image='', phone_number='',
                current_visit_time='', website='', changes='', lastVisited=''):
        out_dict = {
            'URL': url,
            'address': address,
...
def updateLocalObject(name, local_obj):
    ...
    if len(local_obj['business_name'][name]['timeVisited']) > 0 and next(
            iter(local_obj['business_name'][name]['timeVisited'])) != timeStamp:
        ...
        local_obj['business_name'][name]['changeSinceLastVisited'][timeStamp]['google_serp_url'] = res[
            'google_serp_url']
